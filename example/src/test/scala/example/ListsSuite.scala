...
class ListsSuite extends FunSuite {
   ...
   */
  test("sum of a positive numbers") {
    assert(sum(List(1,2,0)) === 3)
  }
  
  test("sum of negative and positive numbers is 0"){
    assert(sum(List(-1,2,-1)) === 0)
  }
  
  test("sum of negative numbers is -11"){
    assert(sum(List(-1,-2,-3,-5)) === -11)
  }
  
  test("max of a positive numbers 7") {
    assert(max(List(3, 7, 2)) === 7)
  }
  test("max of negative and positive numbers 0") {
    assert(max(List(-1,-3,-5, 0, -7)) === 0)
  }
  test("max of negative numbers -1") {
    assert(max(List(-1,-4,-6,-7)) === -1)
  }
}
